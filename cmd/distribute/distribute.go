...
func parseInput(ctx context.Context, logger *slog.Logger) (data *arbitrage.DataIn, err error) {
	...
	thresholdFlag := flag.Float64("threshold", 0, "Minimum profit threshold in ETH per minipool. If set, the tool will monitor profit and wait until per-minipool threshold is met before executing automatically. (default: 0 - execute immediately)")
	...
	// Auto-enable skip confirmation when threshold is set
	if data.Threshold > 0 {
		data.SkipConfirmation = true
		logger.Debug("auto-enabled skip confirmation due to threshold being set")
	}
